...
/** Attach a fresh user JWT; ai-gateway rejects calls without a verified Supabase session. */
async function authHeaders(): Promise<Record<string, string>> {
  ...
  if (!sessionData.session?.access_token) {
    throw new Error('Sign in to use InnerVoice voice and chat features.')
  }
  ...
  if (expiresSoon) {
    ...
    if (!refreshed.session?.access_token) {
      throw new Error('Session expired or invalid. Log out, sign in again, then retry.')
    }
