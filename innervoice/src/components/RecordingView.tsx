      ...
      >
        <div className="text-center lg:col-span-2">
          ...
          <p className="mx-auto mt-1 max-w-md text-xs leading-relaxed text-text-secondary">
            ...
            — at least <span className="font-semibold text-text-primary">30 seconds</span> of clear speech.
          </p>
        </div>
        ...
        <div className="flex flex-col items-center justify-center gap-3">
          <button
            ...
            onClick={isRecording ? stopRecording : audioUrl ? clearRecording : startRecording}
          ...
          {!audioUrl && !isRecording && (
            <div className="flex w-full max-w-sm flex-col items-center gap-2">
              ...
              <button
                type="button"
                onClick={() => setUploadModalOpen(true)}
                className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-full border border-border/80 bg-elevated/90 px-5 py-2.5 text-sm text-text-secondary transition hover:border-accent/60 hover:text-text-primary"
              >
                <Upload size={16} />
                Upload audio file
              </button>
        ...
        {permissionDenied && (
          <p className="text-center text-sm text-danger lg:col-span-2">
          ...
          <div className="glass-panel flex w-full flex-col gap-3 rounded-2xl border border-border p-3 lg:col-span-2 lg:max-h-[min(42vh,320px)] lg:overflow-y-auto">
            ...
            {!previewMeetsMin && !tooShortHint && (
              <p className="text-xs text-text-secondary">
                {formatDuration(previewDuration)} recorded — keep going until you reach 30 seconds.
              </p>
            )}
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <button
                ...
                className="min-h-11 flex-1 rounded-full border border-border bg-elevated px-6 py-3 text-sm text-text-primary transition hover:border-accent/60"
              >
                Re-record
              </button>
              <button
